      ...
      <div class="auth-page">
        <div class="auth-card">
          ...
          <form id="login-form" class="auth-form" novalidate>
            ...
            <button type="button" class="btn btn-block btn-sm mt-1" style="background:#f0f1f3;color:var(--text2)" id="quick-login-btn">快速体验（平台管理员）</button>
    ...
    API.post('/auth/google-login', { credential: response.credential })
      .then(function(res) {
        ...
        if (res.success && res.data) {
          ...
          App.updateNav();
          window.location.hash = App._getDefaultRouteForUser(res.data.user);
        } else {
  ...
  async _doLogin() {
    ...
    try {
      ...
      if (res.success && res.data?.token) {
        ...
        const dest = App._getDefaultRouteForUser(res.data.user);
