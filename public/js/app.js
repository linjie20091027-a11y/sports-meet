const App = {
  ...
  async _showNotifications() {
    try {
      ...
      html += `</div><div class="modal-footer">${data.length>0?`<button class="btn btn-outline btn-sm" onclick="App._markAllRead()">全部标记已讀</button>`:''}<button class="btn btn-primary btn-sm" onclick="App.hideModal()">关闭</button></div>`;
      this.showModal(html);
