...
### [linjie20091027-a11y](https://github.com/linjie20091027-a11y) · 89 提交
**项目负责人 & 全栈架构师**

- **系统架构**：Express + SQL.js 全栈基础搭建、14 张数据表设计、JWT 认证体系、7 层安全防护（Helmet + 速率限制 + bcrypt + 验证码 + 参数化查询 + 审计日志 + 账号锁定）
- **🤖 AI 智能审核系统**：集成 DeepSeek V4 Pro 论坛文字审核 + 豆包(Volcengine Ark) 图片审核双引擎，含管理端开关、审核日志页面、base64 编码适配、响应解析修复
- **🤖 AI 赛程编排**：DeepSeek V4 Pro 智能排程 + pdfkit PDF 导出，含参赛者分组和每轮人员明细
- **📸 精彩瞬间**：6 格轮播布局 + 一键多图上传(最多 9 张) + 全屏灯箱查看 + 上传者追踪 + 审核机制
- **🗣️ 论坛系统**：图片上传/审核/删除 + 回复区紧凑化 + 全站下拉刷新 + 事件委托修复
- **🎨 典藏交互效果**：3D 透视卡片、自定义光标、滚动揭幕、涟漪按钮、数字滚动动画、流光按钮
- **🛠️ 运维部署**：Dockerfile 多阶段构建、docker-compose 一键编排、PM2 进程管理、Nginx 反向代理 + HTTPS、deploy.sh 自动部署脚本
- **📋 管理后台**：控制台仪表盘、用户管理(增删改查/批量导入/权限修复)、赛事/赛程/成绩/公告/论坛/照片/AI审核日志 共 11 个管理模块
- **🔧 故障修复**：Admin 权限误设修复、muted_until 空字符串异常修复、数据库 Key 丢失恢复、论坛 html 变量未声明崩溃修复、init.js 重复 ALTER TABLE 冲突修复

### [whe047319-tech](https://github.com/whe047319-tech) · 43 提交
**前端开发 & 交互体验设计师**

- **报名流程优化**：取消报名需填写理由并提交人工审核、成功后通知学生；报名限制每班每项限 2 人
- **论坛审核接入**：论坛发帖 AI 自动审核集成、被举报自动转人工审核、修复发帖正文空判断(用 innerText 替代 innerHTML)
- **管理后台增强**：年级/班级增删改为弹窗模式(添加 + 删除功能) 、成绩管理筛选改为弹窗(增加项目筛选) 、报名详情查看功能(学号/姓名/班级/项目/性别完整信息)
- **UI/UX 改进**：校徽更换为濠江中学实标、返回键移至导航栏 Logo 右侧(悬浮样式) 、全局返回键(所有子页面顶部) 、比赛 5 分钟检录提醒功能
- **数据统计优化**：控制台报名统计改为环形图、报名热度实时环形图(从全部报名数据计算) 、报名统计表增加状态分布卡片和项目明细
- **通知系统**：修复通知面板标记已读和查看按钮无法点击、登录注册邮箱自动补全 @hkms.hktedu.com

### [ljingxi154-code](https://github.com/ljingxi154-code) · 34 提交
**后端开发 & 数据工程师**

- **数据体系重构**：奖项名称全栈重命名(一等→第一名、二等→第二名、三等→第三名)含数据库迁移；users 表字段补全(sport_group/gender/age/avatar) ；数据库迁移脚本整合
- **成绩展示系统**：ABCDE 五组冠军竖条轮播展示 + 冠军卡片、成绩公示 API 修复(JOIN 缺失) 、loading 状态修复
- **报名系统增强**：性别报名限制(女生只能报女子/混合项目、男生只能报男子/混合项目，前后端双向校验) ；取消报名改为审核流程(学生申请→管理员审批→通知结果)
- **赛事项目页改版**：列宫格布局 + 运动背景图 + 红色蒙层 + 白色图标，Hover 显示原图
- **首页重构**：卡片式公告展示(置顶金色边框优先 + 分类标签) 、赛事卡片 + 公告卡片 + 冠军卡片三区布局
- **搜索功能重构**：卡片式结果布局 + 快捷导航 + 实时搜索 + 扩展搜索类型(赛程/成绩/论坛)
- **精彩瞬间模块**：图片上传审核 + 首页翻页轮播 + 示范照片数据填充
- **Bug 修复**：interactions.js 多余 `})();` 语法错误修复、app.js 重复加载导致旧缓存覆盖修复、loading 遮罩阻挡弹窗关闭修复、公告渲染空白修复

### [Noah](https://github.com/noah200910070082) · 25 提交
**全栈开发 & 架构设计师**

- **教师端工作区**：教师前端 workspace + 后端 workflow + 班主任权限(仅管理任教班级) + 学生自访问限制
- **云端数据库方案**：云数据库迁移工具包(migrate-local-db-to-cloud.js) + 本地数据库同步脚本 + 云端连通性检查
- **权限控制系统**：统一用户资料访问权限(UAP) 、权限角色持久化存储、Admin 注册审核流程退役
- **搜索系统升级**：全场景搜索完善(用户/赛事/公告/成绩/论坛/精彩瞬间) + 好友申请全流程 + 搜索结果修复
- **管理后台升级**：Forum 和 Homepage 筛选器升级、成绩公示布局优化
- **运动组排序统一**：sport-group-order 工具模块

> 📊 共 **195+ 次提交**，4 位核心开发者协作完成。
